...
实现预约流程的状态机管理：
  SEARCHING_PACKAGE -> SELECTING_PACKAGE -> SELECTING_DATE -> CONFIRMING_BOOKING -> CREATING_BOOKING -> BOOKING_CREATED
...
def _preserve_booking_slots(
    result: dict[str, Any],
    resolved_slots: dict[str, Any],
) -> dict[str, Any]:
    """Keep the selected package binding across every booking workflow stage."""
    metadata = result.setdefault("metadata", {})
    task_state = metadata.get("task_state")
    if not isinstance(task_state, dict):
        return result

    stage_slots = task_state.get("slots")
    if not isinstance(stage_slots, dict):
        stage_slots = {}
    task_state["slots"] = {
        **{key: value for key, value in resolved_slots.items() if value not in (None, "", [])},
        **stage_slots,
    }
...
def build_booking_appointment_datetime(
    ...
    """Convert a booking date to the internal noon timestamp used by orders."""
    parsed_date = parse_date_string(str(appointment_date or ""), _today())
    if parsed_date is None:
        raise ValueError("日期格式无效")

    # `appointment_time` remains accepted for old callers, but is deliberately ignored.
...
def _parse_time_range(range_str: str) -> tuple[int, int] | None:
    """Compatibility helper for legacy callers; booking no longer uses hourly ranges."""
    return _availability_parse_time_range(range_str)


def _format_minutes(minutes: int) -> str:
    """Compatibility helper for legacy callers; booking no longer exposes hourly ranges."""
...
def get_available_slots(
    ...
    *,
    ...
    """
    # The legacy agent API keeps its name for compatibility, but it now
    # returns a date-level availability marker instead of hourly slots.
    ...
    availability = list_bookable_slots(db, profile, query_date, 1, package_duration)
    day = (availability.get("days") or [{}])[0]
    is_bookable = bool(day.get("bookable"))
    return {
        "photographer_id": photographer_id,
        "photographer_name": profile.user.display_name if profile.user else None,
        ...
        "slots": [{"date": query_date.isoformat(), "label": "可预约"}] if is_bookable else [],
        "package_duration": package_duration,
        "total_available": 1 if is_bookable else 0,
        "bookable": is_bookable,
        "unavailable_reason": day.get("unavailable_reason"),
    }
...
def find_nearby_available_dates(
    ...
    *,
    ...
    """
    查找摄影师未来 N 天内可预约的所有日期。
    档期默认为空闲，只有忙碌日期和已有订单会排除可预约日期。
    """
    results = []
    ...
    for message in messages:
        ...
        if "最早" in text:
            def earliest(pkg: dict) -> str:
                ...
                return str(slots[0].get("date")) if slots else "9999"
...
def _format_slot_summary(slots_data: dict[str, Any]) -> str:
    """Format the legacy availability response without exposing hourly slots."""
    date_label = slots_data.get("date_label", "")
    slots = slots_data.get("slots") or []
    if not slots:
        return f"{date_label} 暂无可预约日期"
    return f"{date_label} 可以预约，无需选择具体时刻"
...
def _latest_booking_task_state(
    ...
    if user_id is not None:
        ...
        if task and task.task_type == "create_booking":
            ...
            legacy_appointment_time = fields.get("appointment_time")
            if legacy_appointment_time and not slots.get("date"):
                try:
                    parsed = datetime.fromisoformat(str(legacy_appointment_time))
...
def _merge_booking_slots(task_state: dict | None, content: str | None) -> dict[str, Any]:
    ...
    if task_state:
        ...
        for key in (
            ...
            "package_display", "selected_package", "date", "appointment_date",
    ...
    if content:
        ...
        for key, val in extracted.items():
            if key in {"time", "appointment_time"}:
                continue
            if val not in (None, "", []):
                slots[key] = val

    # Historical task states may still contain a time; it is never part of the
    # current booking contract.
    slots.pop("time", None)
    slots.pop("appointment_time", None)
...
def _ask_for_date_result(
    ...
    *,
    ...
    if not nearby:
        return {
            "content": (
                f"你选择的 {pkg_display} 未来两周暂时没有可预约的日期。"
                "你可以换个摄影师或稍后再查看。"
            ),
            "metadata": {
    ...
    content_parts = [
        f"你选择了 {pkg_display}。",
        "请告诉我你想约哪一天？以下是近期可选日期，预约不需要选择具体时刻。",
    ]
...
def _ask_for_time_result(
    ...
    *,
    ...
    if not slot_result.get("slots"):
        return {
            "content": (
                f"{slot_result.get('date_label', date_str)} {pkg_display} 没有可预约日期。"
                "你可以选另一个日期，我帮你再查。"
            ),
            "metadata": {
                "model": {"provider": "platform_orchestrator", "model": "booking-agent-no-date"},
                "suggested_actions": [],
                "task_state": {
                    "task_type": "create_booking",
                    "status": "awaiting_date",
...
def _confirmation_result(
    *,
    ...
    build_booking_appointment_datetime(appointment_date)
    booking_payload = {
...
def _execute_booking_result(
    ...
    *,
    ...
    """用户确认后创建预约"""
    try:
        appointment_dt = build_booking_appointment_datetime(appointment_date)
            ...
            "task_state": {
                ...
                "slots": {
                    ...
                    if k in ("photographer_id", "photographer_name", "package_display",
                             "date", "appointment_date", "duration_minutes")
                },
                "pending_action": None,
            },
        },
